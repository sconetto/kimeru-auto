    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      <section className="mb-10 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 p-8 text-white sm:p-12">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <Link
            href="/compare"
            className="inline-flex items-center justify-center rounded-md bg-white px-3 py-2.5 text-center text-sm font-semibold text-blue-700 transition-colors hover:bg-blue-50 sm:px-5"
          >
            {t("compareCars")}
          </Link>
          <Link
            href="/financing"
            className="inline-flex items-center justify-center rounded-md border border-white/40 px-3 py-2.5 text-center text-sm font-semibold text-white transition-colors hover:bg-white/10 sm:px-5"
          >
            {t("simulateFinancing")}
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
          {brands.map((brand) =>
            brand.modelCount === 0 ? (
              <div
                key={brand.id}
                className="flex flex-col items-center justify-center gap-3 rounded-lg border border-slate-200 bg-slate-50 p-6 opacity-60 grayscale dark:border-slate-800 dark:bg-slate-800/40"
                aria-disabled="true"
              >
